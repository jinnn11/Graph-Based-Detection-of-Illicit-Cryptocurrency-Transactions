"""Train a temporal GATv2 model with time encoding."""
...
from src.models.temporal_gnn import build_temporal_gat
...
class LoadedData:
    ...
    val_mask: torch.Tensor
    test_mask: torch.Tensor
    graph_normalized: bool
    graph_reverse_edges: bool
...
def prepare_data(config: TrainConfig, device: torch.device) -> LoadedData:
    ...
    train_mask, val_mask, test_mask = build_labeled_masks(
        y, graph.train_mask, graph.val_mask, graph.test_mask
    )

    x = graph.x.to(device)
    edge_index = graph.edge_index.to(device)
    time_step = graph.time_step.to(device)
    y = y.to(device)
...
def run_experiment(config: TrainConfig, cached: LoadedData | None = None) -> Dict[str, float]:
    ...
    model = build_temporal_gat(
        ...
        heads=config.heads,
        max_time=int(time_step.max().item()),
    ).to(device)
    ...
    for epoch in range(1, config.epochs + 1):
        loss = train_epoch(model, optimizer, x, edge_index, time_step, y, train_mask, loss_fn)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Temporal GATv2 baseline")
    ...
    parser.add_argument("--output", default="experiments/tgat_results.json")
    parser.add_argument("--model-output", default="experiments/tgat_model.pt")
